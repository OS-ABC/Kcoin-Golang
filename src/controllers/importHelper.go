...
func ImportProject(url string)error{
	...
	for _,singleUser:=range users{
		...
		var k_user_id int
		var project_id int
		res, err := o.Raw(querySql, singleUser).Exec()
		if err!=nil{
			...
			if num==0{//如果没查到
				...
				err := o.Raw(`select k_user_id from "K_User" where USER_NAME=?`,singleUser).QueryRow(&k_user_id)
				if err!=nil{
					log.Fatal("when query k_user_id in K_User,error occured:",err)
				}
				//然后根据根据项目名查K_Project表获取project_id
				err = o.Raw(`select project_id from "K_Project" where project_name=?`,repoName).QueryRow(&project_id)
				if err!=nil{
					log.Fatal("when query project_id in K_Project ,error occured:",err)
				}
				//最后两个id都有了，插入K_user_in_project
				res,err = o.Raw(insertSql,project_id,k_user_id).Exec()
				if err==nil{
